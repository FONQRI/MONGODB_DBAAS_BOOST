				   ...
				   http::server::request request)
{
	...
	try {
		...
		if (request.method == "POST") {
			...
			// update_name
			optional_string update_name;
			try {
				update_name = request_document.view()["update_name"]
						  .get_utf8()
						  .value.to_string();
			}
			catch (std::exception &e) {
			...
			// request_per_day
			optional_int request_per_day;
			try {
				request_per_day =
				request_document.view()["request_per_day"]
					.get_int32();
			}
			catch (std::exception &e) {
