...
void dbaas::controller::find_one_and_replace(http::server::reply &rep,
						 http::server::request request)
{
	...
	try {
		...
		if (request.method == "POST") {
			...
			// get request document from request document
			bsoncxx::types::b_document replacement;

			try {

				replacement = request_document.view()["replacement"]
						  .get_document();
			}
			catch (std::exception &e) {
			...
			username, database_name, filter, replacement,
			projection, sort, collation, acknowledge_level, tag,
			journal, majority, timeout, nodes, max_time, upsert,
			bypass_document_validation);
